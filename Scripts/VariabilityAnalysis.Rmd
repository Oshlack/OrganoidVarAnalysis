...
I will assume that the time series analsis hasn't been run and will redo the data processing here before the analysis.
...
Here we use the time series data as a baseline to compare against in order to predict the age of the organoids from Batches 1, 3, 4 and 5.
...
```{r}
write.csv(results.sc6,file="VarCompResults-SC6-06092018.csv",row.names=FALSE)
```
...
write.csv(topGO(goana(de=results.sc6.vial$gene_id[1:100],universe=y.sc6.d18$genes$gene_id, species="Hs",covariate = length)),file="sc6-vialgenes-GO.csv")


# top 100 most variable residual
results.sc6.res <- results.sc6[results.sc6$MaxVarExplained=="PropRes",]
write.csv(topGO(goana(de=results.sc6.res$gene_id[1:100],universe=y.sc6.d18$genes$gene_id, species="Hs",covariate = length)),file="sc6-residgenes-GO.csv")
...
pdf(file="Z:/Group-Little_MCRI/Manuscripts/In Progress/Phipson 2017 Organoid transcriptional profiling/Figure6/barcodeplot-sc6vsc32.pdf",height = 5,width=8)
...
o <- order(grp.all)
aheatmap(logcpm.all[de.genes.ts,o],labCol=targets.all2$age[o],annCol=list(Batch=factor(targets.all2$Batch[o]),geno=targets.all2$geno2[o]),fontsize = 12,Colv = NA)
```
...
```{r}
# NPHP3 data
counts.nphp3 <- read.delim("D:/_MCRI/_Analysis projects/MelissaLittle/NPHP3_disease_modelling/counts.txt",skip=1,header=TRUE)
targets.nphp3 <- read.table("D:/_MCRI/_Analysis projects/MelissaLittle/NPHP3_disease_modelling/targets.txt",header=TRUE,stringsAsFactors=FALSE)
...
```{r}
# Lorna's data which is mapped to hg38
# Replace data.lorna with hg19 mapped counts
#data.lorna <- read.delim("Z:/Group-Little_MCRI/Data/RNA-Seq/RawData/Lorna-podocyte/aligned_reads_star/counts_gencodeV20.txt",skip=1,header=TRUE)

data.hg19 <- read.delim("Z:/Group-Little_MCRI/Data/RNA-Seq/RawData/Lorna-podocyte/aligned_reads_star_hg19/counts_hg19.txt",skip=1,header=TRUE)

counts.lorna <- data.hg19[,-(1:6)]
rownames(counts.lorna) <- data.hg19[,1]
colnames(counts.lorna) <- gsub("_ML170410_170322_HCVCFBGX2_NB501544_L000_Aligned.sortedByCoord.out.bam","",colnames(counts.lorna))
colnames(counts.lorna) <- gsub("_ML170411_170323_HCTFWBGX2_NB501544_L000_Aligned.sortedByCoord.out.bam","",colnames(counts.lorna))

targets.lorna <- read.delim("Z:/Group-Little_MCRI/Data/RNA-Seq/RawData/Lorna-podocyte/analysis/targets.txt",stringsAsFactors = FALSE,sep="\t")

counts.c32.ltl <- counts.lorna[,targets.lorna$CellType=="C32" & targets.lorna$Source=="LTL"]

table(rownames(counts.c32.ltl) == rownames(y.c32.sc6.nphp3))

m <- match(rownames(y.c32.sc6.nphp3),rownames(counts.c32.ltl))

y.c32.sc6.nphp3.ltl <- DGEList(cbind(ts[!is.na(m),-(1:6)],early[!is.na(m),-(1:6)],data[!is.na(m),],counts[!is.na(m),],data.nphp3[!is.na(m),],counts.c32.ltl[m[!is.na(m)],]))
rownames(y.c32.sc6.nphp3.ltl) <- rownames(counts.c32.ltl[m[!is.na(m)],])
...
pdf(file="D:/_MCRI/_Analysis projects/MelissaLittle/NatureMethods/Figure6/epithelial-stroma-genes-hg19-updated.pdf",height = 10,width=22)
par(mfrow=c(2,5))
par(mar=c(6.5,5,3,2))
...
pdf(file="D:/_MCRI/_Analysis projects/MelissaLittle/NatureMethods/Figure6/epithelial-stroma-genes-hg19-updated.pdf",height = 10,width=23)
par(mfrow=c(2,5))
par(mar=c(8.5,5,3,2))
